...
class Trainer:
    ...
    def fit(self, train_loader, val_loader=None):
        ...
        for epoch in tqdm(range(self.n_epochs)):
            ...
            for x, y in train_loader:
                ...
                # attention_batch.append(att.cpu().detach().numpy())
                attention_batch.append(att.tolist())

            forecast_b_losses = np.array(forecast_b_losses)
            recon_b_losses = np.array(recon_b_losses)
            attention_epoch.append(attention_batch[-1])
        ...
        self.save_att(f"train_att.pkl", attention_epoch)
    ...
    def evaluate(self, data_loader):
        ...
        with torch.no_grad():
            for x, y in data_loader:
                ...
                # attention_epoch.append(att.cpu().detach().numpy())
                attention_epoch.append(att.tolist())

        forecast_losses = np.array(forecast_losses)
        recon_losses = np.array(recon_losses)
        

        self.save_att(f"test_att.pkl", attention_epoch)
    ...
    def save_att(self, file_name, file):
        PATH = self.dload + "/" + file_name
        if os.path.exists(self.dload):
            pass
        else:
